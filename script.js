...
const providerOptions = {
  walletconnect: {
    ...
    options: {
      ...
      chainId: 56,
      qrcodeModalOptions: {
        mobileLinks: ["trust", "metamask", "binance", "coinbase"]
      }
    }
  }
};
...
// BNB girince FDAI hesaplama
bnbAmountInput.addEventListener("input", () => {
  const bnb = parseFloat(bnbAmountInput.value);
  tokenAmount.textContent = !isNaN(bnb) && bnb > 0
    ? `${(bnb * TOKENS_PER_BNB).toLocaleString()} FDAI`
    : "0 FDAI";
});
...
buyBtn.addEventListener("click", async () => {
  ...
  try {
    const valueInWei = web3.utils.toWei(bnb.toString(), "ether");
